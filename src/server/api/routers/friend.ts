...
import { Friend, User, Lineup } from "~/server/models";
...
export const friendRouter = createTRPCRouter({
  ...
  searchFriends: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      let friends = currentUser.friends as any[];
  ...
  getMyFriends: protectedProcedure.query(async ({ ctx }) => {
    const currentUser = await User.findById(ctx.session.user.id).populate(
      "friends",
    ...
    return (currentUser.friends as any[]).map(transformUser);
  }),
  ...
  getFriendLineups: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const lineups = await Lineup.find({ ownerId: friendId })
        .sort({ createdAt: -1 })
  ...
  getAllFriendsLineups: protectedProcedure.query(async ({ ctx }) => {
    ...
    const lineups = await Lineup.find({ ownerId: { $in: friendIds } })
      .sort({ createdAt: -1 })
      .populate(lineupPopulateFields);

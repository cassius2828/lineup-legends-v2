...
export function ProfileBannerSkeleton() {
  return <div className="bg-foreground/5 h-48 animate-pulse md:h-100" />;
}

export function ProfileAvatarSkeleton() {
  return (
    <div className="relative -mt-16 mb-4 flex justify-center md:justify-start">
      <div className="border-surface-950 bg-foreground/10 h-32 w-32 animate-pulse rounded-full border-4" />
    </div>
  );
}
...
export function ProfileHeaderSkeleton() {
  return (
    <div className="mb-6 flex animate-pulse flex-col items-center gap-4 md:flex-row md:items-start md:gap-6">
      <div className="flex-1 space-y-3 text-center md:text-left">
        <div className="bg-foreground/10 mx-auto h-8 w-48 rounded md:mx-0" />
        <div className="bg-foreground/10 mx-auto h-4 w-28 rounded md:mx-0" />
        <div className="bg-foreground/10 mx-auto h-4 w-64 rounded md:mx-0" />
      ...
      <div className="flex gap-3">
        <div className="bg-foreground/10 h-9 w-24 rounded-lg" />
      </div>
    </div>
  );
}
...
export function ProfileStatsGridSkeleton() {
  return (
    <div className="mb-8 grid animate-pulse grid-cols-2 gap-3 md:grid-cols-4">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="bg-foreground/5 rounded-xl p-4 text-center">
          <div className="bg-foreground/10 mx-auto h-7 w-12 rounded" />
          <div className="bg-foreground/10 mx-auto mt-2 h-3.5 w-20 rounded" />
        </div>
      ))}
    </div>
  );
...
export function ProfileLineupsSkeleton({
  view = "list",
}: { view?: ViewMode } = {}) {
  return (
    <div className="pb-16">
      <div className="mb-4 space-y-2">
        ...
        <div className="flex gap-2">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="bg-foreground/10 h-8 w-20 animate-pulse rounded-lg"
            />
          ))}
        </div>
      </div>
      <LineupListSkeleton view={view} count={view === "grid" ? 12 : 3} />
    </div>
  );
}

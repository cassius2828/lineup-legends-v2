...
The user profile system lets users view and customize their public profile, browse lineup statistics, upload images, and follow other users.
...
| Highest Rated | The user's lineup with the highest `avgRating` (links to that lineup) |
...
- Clickable image upload fields for both the banner and profile image (replaces the old URL text inputs)
...
- A search input with 300ms debounce queries users by name or username
...
| --- | --- | --- | --- |
| `follow.toggleFollow` | Mutation | Protected | Follow or unfollow a user |
...
| `follow.searchUsers` | Query | Public | Search users by name or username |
...
| Endpoint | Type | Auth | Description |
| --- | --- | --- | --- |
| `profile.getById` | Query | Public | Full profile with lineups, stats, and featured lineups |
| `profile.getMe` | Query | Protected | Current user's profile data |
...
The `getById` endpoint runs lineup queries and the stats aggregation in parallel for performance.
